...
#warning gain.dot(this->calc(t, gain)) in bspline.l?
  ...
  void varifySumOfBSplineVector(const hrp::dvector& gain)
  {
    ...
    double stddev = std::sqrt(std::accumulate(bufs.begin(), bufs.end(), 0.0,
                                              [average](double sum, const double& e) { return sum + (e - average) * (e - average); }) /
                              bufs.size());
...
private:
  ...
  {
    std::vector<DiscreteDeltaMatrix>::iterator itr = std::find_if(discrete_delta_matrix_.begin(), discrete_delta_matrix_.end(),
